...
class _InventoryScreenState extends ConsumerState<InventoryScreen> {
  ...
  Widget _buildContent(InventoryState state, AppLocalizations l10n) {
    ...
    if (state.error != null && state.items.isEmpty) {
      return Center(
        child: Text(
          l10n.errorLoadingData(state.error!),
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return PopupMenuButton<int>(
      ...
      onSelected: (categoryId) {
        ref
            .read(inventoryFilterProvider.notifier)
            .update((state) => state.copyWith(categoryId: categoryId));
      },
      itemBuilder: (context) => categoriesAsync.when(
        loading: () => [
          PopupMenuItem(enabled: false, child: Text(l10n.loading)),
        ],
        error: (e, s) => [
          PopupMenuItem(enabled: false, child: Text(l10n.error)),
        ],
        data: (categories) => [
          PopupMenuItem<int>(value: -1, child: Text(l10n.allCategories)),
          ...categories.map(
            (cat) => PopupMenuItem<int>(value: cat.id, child: Text(cat.name)),
          ),
        ],
      ),
      child: _FilterButton(
          ...
          data: (categories) {
            ...
            return categories
                .firstWhere(
                  ...
                  orElse: () => Category(id: -1, name: l10n.categoryLabel),
                )
                .name;
          },
        ),
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return PopupMenuButton<int>(
      ...
      onSelected: (warehouseId) {
        ref
            .read(inventoryFilterProvider.notifier)
            .update((state) => state.copyWith(warehouseId: warehouseId));
      },
      itemBuilder: (context) => warehousesAsync.when(
        ...
        data: (warehouses) => [
          ...
          ...warehouses.map(
            (w) => PopupMenuItem<int>(value: w.id, child: Text(w.name)),
          ),
        ],
      ),
      child: _FilterButton(
        text: warehousesAsync.when(
          ...
          data: (warehouses) {
            ...
            return warehouses
                .firstWhere(
                  (w) => w.id == currentFilter.warehouseId,
                  orElse: () => Warehouse(id: -1, name: l10n.locationLabel),
                )
                .name;
          },
        ),
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return PopupMenuButton<String>(
      ...
      onSelected: (statusValue) {
        ref
            .read(inventoryFilterProvider.notifier)
            .update((state) => state.copyWith(stockStatus: statusValue));
      },
      itemBuilder: (context) => StockStatus.values
          .map(
            (status) => PopupMenuItem<String>(
              value: status.value,
              child: Text(getLocalizedStatusName(status)),
            ),
          )
          .toList(),
      child: _FilterButton(
        text: getLocalizedStatusName(
          StockStatus.values.firstWhere(
            (s) => s.value == currentFilter.stockStatus,
          ...
          child: Row(
            ...
            children: [
              Expanded(
                ...
                child: Text(
                  isHeader
                      ? l10n.tableItemName.toUpperCase()
                      : item!.productName,
                  style: isHeader ? headerStyle : primaryStyle,
              ...
              Expanded(
                ...
                child: Text(
                  isHeader
                      ? l10n.tableCategory.toUpperCase()
                      : item!.categoryName,
                  style: isHeader ? headerStyle : regularStyle,
                  overflow: TextOverflow.ellipsis,
                ),
              ),
              Expanded(
                flex: 2,
                child: Text(
                  isHeader ? l10n.tableSku.toUpperCase() : item!.sku,
                  style: isHeader ? headerStyle : regularStyle,
                  overflow: TextOverflow.ellipsis,
                ),
              ),
              Expanded(
                ...
                child: Text(
                  isHeader
                      ? l10n.tableQuantity.toUpperCase()
                      : '${item!.quantityOnStock} ${item!.unitName}',
                  style: isHeader ? headerStyle : regularStyle,
                  overflow: TextOverflow.ellipsis,
                ),
              ),
              Expanded(
                ...
                child: Align(
                  ...
                  child: isHeader
                      ? Text(l10n.tableStatus.toUpperCase(), style: headerStyle)
                      : _StatusChip(
                          inStock:
                              (double.tryParse(item!.quantityOnStock) ?? 0) > 0,
                        ),
                ),
              ),
              Expanded(
...
class _StatusChip extends StatelessWidget {
  final bool inStock;
  const _StatusChip({required this.inStock});
  @override
  Widget build(BuildContext context) {
    final l10n = AppLocalizations.of(context)!;
    return Chip(
