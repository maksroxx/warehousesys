  ...
  Future<void> _importExcel(BuildContext context, WidgetRef ref) async {
    try {
      final result = await FilePicker.platform.pickFiles(
        ...
        dialogTitle: "Выберите прайс-лист для импорта",
      );

      if (result != null && result.files.single.path != null) {
        final file = File(result.files.single.path!);
        AppSnackbars.showInfo("Загрузка файла, пожалуйста подождите...");
        await ref.read(stockRepositoryProvider).importProducts(file);

        if (context.mounted) {
          AppSnackbars.showSuccess("Товары успешно импортированы!");
          ref.invalidate(dashboardDataProvider);
        }
      }
    } catch (e) {
      if (context.mounted) {
        AppSnackbars.showErrorObject(e);
      }
    }
  }
                ...
                data: (data) => Column(
                  children: [
                      ...
                      children: [
                        Expanded(
                          child: _StatCard(
                            ...
                            color: Colors.blue,
                            subtitle:
                                "На ${data.itemsInStock} активных позициях",
                          ),
                        ),
                        ...
                        Expanded(
                          child: _StatCard(
                            ...
                            value:
                                "+${double.parse(data.incomingToday).toStringAsFixed(0)}",
                            secondaryValue:
                                "-${double.parse(data.outgoingToday).toStringAsFixed(0)}",
                    ...
                    Row(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Expanded(flex: 2, child: _DynamicsSection(data: data)),
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              _QuickActionsCard(
                                context: context,
                                l10n: l10n,
                                onImportTap: () => _importExcel(context, ref),
                              ),
                            ],
                          ),
                        ),
        ...
        Column(
          ...
          children: [
            ...
            Text(
              ...
              style: Theme.of(
                context,
              ).textTheme.bodyMedium?.copyWith(color: textGreyColor),
            ),
          ],
        ),
          ...
          data: (warehouses) => Container(
            ...
            child: DropdownButtonHideUnderline(
              child: DropdownButton<int?>(
                ...
                onChanged: (val) =>
                    ref
                            .read(selectedDashboardWarehouseProvider.notifier)
                            .state =
                        val,
              ),
            ),
            ...
            return Column(
              ...
              children: [
                Row(
                  ...
                  children: [
                    _AnimatedBar(
                      heightFactor: hFactorIn,
                      color: primaryColor,
                      value: income,
                    ),
                    const SizedBox(width: 4),
                    _AnimatedBar(
                      heightFactor: hFactorOut,
                ...
                Text(
                  dateLabel,
                  style: const TextStyle(fontSize: 12, color: textGreyColor),
                ),
              ],
            );
          }).toList(),
      ...
      children: [
        Row(
          ...
          children: const [
            Text(
              "Меньше",
              style: TextStyle(fontSize: 10, color: textGreyColor),
            ),
            SizedBox(width: 8),
        ...
        GridView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            if (isToday) {
              ...
              final opacity = (activity / maxActivity)
                  .clamp(0.15, 0.6)
                  .toDouble();
